...
- `RegionEditorModal` owns viewport-aware dialog UX, snippet zoom/protection controls, current-page bbox navigation wiring (`First`/`Previous`/order input/`Next`/`Last`), dialog-scoped region-navigation shortcut gating/routing, and outer-pane separator rendering for drag/keyboard resize.
